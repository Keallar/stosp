  ...
  describe '#available_distributors' do
    context 'with successfully' do
      let(:response_body) do
        {
          'receive' =>
            [{
              'code' => '39', 'name' => 'Антей-транзит', 'city' => 'Владивосток',
              'address' => 'ул. Енисейская, 32', 'additional_address' => ''
            }],
          'distribute' =>
            [{
      ...
      it 'return response with receive and distribute' do
        expect(client.available_distributors).to include(*response_keys)
      end
    end
  end
end
